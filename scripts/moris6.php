...
function saveTempStateValue($user_id, $state, $temp_name, $temp_username = null) {
    global $pdo;
    $stmt = $pdo->prepare("
        INSERT INTO temp_states (user_id, state, temp_name, temp_username) 
        VALUES (?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
            state = VALUES(state),
            temp_name = VALUES(temp_name),
            temp_username = VALUES(temp_username)
    ");
    $stmt->execute([$user_id, $state, $temp_name, $temp_username]);
}



function getTempStateValue($user_id, $state) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT temp_name FROM temp_states WHERE user_id = ? AND state = ?");
